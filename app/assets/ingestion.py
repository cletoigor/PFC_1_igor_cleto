...
# This module now lives at app/assets/ingestion.py, one directory deeper than the
# original app/assets.py, so the "app dir" is the parent of this file's directory.
...
def raw_tuya_logs(context: AssetExecutionContext, config: TuyaCredentials) -> str:
    ...
    # Resolve the mapping path relative to the app directory
    relative_mapping_path = config.device_mapping_path
    absolute_mapping_path = os.path.abspath(os.path.join(_APP_DIR, relative_mapping_path))
    ...
    device_mapping = load_device_mapping(absolute_mapping_path, logger=context.log)
    ...
    start_time_ms, end_time_ms = get_time_range_ms(TIME_WINDOW_HOURS, logger=context.log)
    ...
    for device_id in device_ids:
        ...
        supported_codes = get_device_supported_codes(openapi, device_id, logger=context.log)
    ...
    # --- Save logs to files ---
    # Ensure base directory exists, relative to the app directory
    absolute_base_output_dir = os.path.abspath(os.path.join(_APP_DIR, BASE_OUTPUT_DIR))
    ...
    if saved_files_count == 0:
        # A run that persists zero raw files is a failed ingestion (empty API
        # responses across every device, auth issues masked as empty logs, etc.)
        # and must fail loudly rather than silently succeed with no data.
        raise RuntimeError(
            "No log files were saved in this run — failing the asset. "
...
def staging_tuya_logs(context: AssetExecutionContext) -> str:
    ...
    staging_dir_abs = os.path.abspath(os.path.join(_APP_DIR, STAGING_DIR))
    device_mapping_path_abs = os.path.abspath(
        os.path.join(_APP_DIR, DEFAULT_PROCESSING_MAPPING_PATH)
    )
